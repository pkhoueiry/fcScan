...
regions is done using the `mapCluster`.
...
###### Window Size(w)
Window size is set using `w`, and this parameter defines the sizes of the
clusters.
...
(Default of `chr` is **NULL**) meaning that clustering on all chromosomes will be
performed
...
it from the terminal using bash or any other alternative to extract the required sites.
...
minimum of 1 NANOG_known1 site and 2 TATA_known2 sites (since greedy is set to TRUE by default),
found within 1000bp, and adjacent clusters should be seperated by a maximum of 5 overlapping base
pairs.
...
clusters. To get only the cluster labelled by *TRUE* We can subset by the following:
